...
do
    ...
    java -XX:+UseConcMarkSweepGC -Xmx3000m -cp ${classpath} BenchNeighbor neighbor-latency \
		${neO4J_DIR}/${num_nodes} \
    ...
    sync && sudo sh -c 'echo 3 > /proc/sys/vm/drop_caches'
    java -XX:+UseConcMarkSweepGC -Xmx3000m -cp ${classpath} BenchNode node-latency \
		${neO4J_DIR}/${num_nodes} \
    ...
    sync && sudo sh -c 'echo 3 > /proc/sys/vm/drop_caches'
    java -XX:+UseConcMarkSweepGC -Xmx3000m -cp ${classpath} MixBench latency \
		${neO4J_DIR}/${num_nodes} \
		${QUERY_DIR}/node_warmup_${num_nodes}.txt \
		${QUERY_DIR}/node_query_${num_nodes}.txt \
		${QUERY_DIR}/neighbor_warmup_${num_nodes}.txt \
    ...
    sync && sudo sh -c 'echo 3 > /proc/sys/vm/drop_caches'
    java -XX:+UseConcMarkSweepGC -Xmx3000m -cp ${classpath} BenchNode node-node-latency \
		${neO4J_DIR}/${num_nodes} \
		${QUERY_DIR}/node_warmup_${num_nodes}.txt \
		${QUERY_DIR}/node_query_${num_nodes}.txt \
    ...
    sync && sudo sh -c 'echo 3 > /proc/sys/vm/drop_caches'
    java -XX:+UseConcMarkSweepGC -Xmx3000m -cp ${classpath} NeighborNodeBench latency \
		${neO4J_DIR}/${num_nodes} \
