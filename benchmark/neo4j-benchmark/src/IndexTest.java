		...
		// END SNIPPET: startDb
		{
			...
			try (Transaction tx = graphDb.beginTx()) {
				...
				indexDefinition = schema.indexFor(DynamicLabel.label("User"))
						.on("username").create();
				index = graphDb.index().forNodes("User");
				tx.success();
			}
		...
		}
		{
			...
			try (Transaction tx = graphDb.beginTx()) {
				...
				for (int id = 0; id < 100; id++) {
					...
					index.add(userNode, "username", userNode.getProperty("username"));
				}
		...
		}
		{
			...
			try (Transaction tx = graphDb.beginTx()) {
				try (ResourceIterator<Node> users = graphDb
						.findNodesByLabelAndProperty(label, "username",
								nameToFind).iterator()) {
					...
					for (Node node : userNodes) {
						System.out.println("The username of user " + idToFind
								+ " is " + node.getProperty("username"));
					}
				}
			}
		...
		}
		{
			...
			try (Transaction tx = graphDb.beginTx()) {
				...
				for (Node node : graphDb.findNodesByLabelAndProperty(label,
						"username", nameToFind)) {
					node.setProperty("username", "user" + (idToFind + 1)
							+ "@neo4j.org");
				}
				tx.success();
			}
