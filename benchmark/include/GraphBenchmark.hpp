...
class GraphBenchmark {
 private:
  ...
  inline int choose_query(double rand_r) {
    if (rand_r < ASSOC_RANGE_PERC) {
      ...
      return 2;
        ...
        < ASSOC_RANGE_PERC + OBJ_GET_PERC + ASSOC_GET_PERC + ASSOC_COUNT_PERC) {
      return 3;
    }
    return 4;
  }
  ...
  void bench_throughput(const int num_threads,
    ...
    switch (type) {
      ...
      case NHBR_NODE:
        ...
        break;
      case NHBR_NODE2:
        LOG_E("Starting nhbrAttr2 thput\n")
        ;
        for (auto thread_data : thread_datas) {
          threads.push_back(
              shared_ptr<std::thread>(
                  new std::thread(
                      &GraphBenchmark::benchmark_neighbor_node2_throughput_helper,
                      this, thread_data)));
        }
        break;
      case NODE:
        ...
        for (auto thread_data : thread_datas) {
          threads.push_back(
              shared_ptr<std::thread>(
                  new std::thread(
                      &GraphBenchmark::benchmark_node_throughput_helper, this,
                      thread_data)));
        }
        break;
      case NODE2:
      ...
      case NODE2:
        ...
        for (auto thread_data : thread_datas) {
          threads.push_back(
              shared_ptr<std::thread>(
                  new std::thread(
                      &GraphBenchmark::benchmark_node_node2_throughput_helper,
                      this, thread_data)));
        }
        break;
      case EDGE_ATTRS:
      ...
      case MIX:
        ...
        for (auto thread_data : thread_datas) {
          threads.push_back(
              shared_ptr<std::thread>(
                  new std::thread(
                      &GraphBenchmark::benchmark_mix_throughput_helper, this,
                      thread_data)));
        }
        break;
      case TAO_ASSOC_RANGE:
        ...
        for (auto thread_data : thread_datas) {
          threads.push_back(
              shared_ptr<std::thread>(
                  new std::thread(
                      &GraphBenchmark::benchmark_tao_assoc_range_throughput_helper,
                      this, thread_data)));
        }
        break;
      case TAO_ASSOC_COUNT:
        ...
        for (auto thread_data : thread_datas) {
          threads.push_back(
              shared_ptr<std::thread>(
                  new std::thread(
                      &GraphBenchmark::benchmark_tao_assoc_count_throughput_helper,
                      this, thread_data)));
        }
        break;
      case TAO_OBJ_GET:
        ...
        for (auto thread_data : thread_datas) {
          threads.push_back(
              shared_ptr<std::thread>(
                  new std::thread(
                      &GraphBenchmark::benchmark_tao_obj_get_throughput_helper,
                      this, thread_data)));
        }
        break;
      case TAO_ASSOC_GET:
        ...
        for (auto thread_data : thread_datas) {
          threads.push_back(
              shared_ptr<std::thread>(
                  new std::thread(
                      &GraphBenchmark::benchmark_tao_assoc_get_throughput_helper,
                      this, thread_data)));
        }
        break;
      case TAO_ASSOC_TIME_RANGE:
        ...
        for (auto thread_data : thread_datas) {
          threads.push_back(
              shared_ptr<std::thread>(
                  new std::thread(
                      &GraphBenchmark::benchmark_tao_assoc_time_range_throughput_helper,
                      this, thread_data)));
        }
        break;
      case TAO_ASSOC_ADD:
...
//        }
  ...
  void read_test_queries(const std::string& warmup_assoc_range_file,
    ...
    // assoc_time_range
    read_assoc_time_range_queries(warmup_assoc_time_range_file,
                                  assoc_time_range_file);
  }
      ...
      switch (query_type) {
        case 0:
          LOG_E("assoc range, query idx %d (%d %d %d %d)\n", query_idx,
                ...
                warmup_assoc_range_offs.size(), warmup_assoc_range_lens.size())
          ;
        ...
        case 1:
          LOG_E("obj_get, query idx %d (%d)\n", query_idx,
                warmup_obj_get_nodes.size())
          ;
          client->obj_get(attrs, this->warmup_obj_get_nodes.at(query_idx));
          break;
        case 2:
          ...
          client->assoc_get(result, this->warmup_assoc_get_nodes.at(query_idx),
        ...
        case 3:
          ...
          ;
          client->assoc_count(this->warmup_assoc_count_nodes.at(query_idx),
        ...
        case 4:
          ...
          client->assoc_time_range(
              result, this->warmup_assoc_time_range_nodes.at(query_idx),
        ...
        case 5:
          ...
          dst = dist_node(gen);
          LOG_E("assoc_add(%lld,atype %d,%lld,...)\n", src, atype, dst)
          ;
          ret = client->assoc_add(src, atype, dst, MAX_TIME,
                                  ATTR_FOR_NEW_EDGES);
          LOG_E("; ret = %d\n", ret)
          ;
          break;
  ...
  void benchmark_neighbor_latency(std::string res_path, uint64_t WARMUP_N,
                                  uint64_t MEASURE_N,
  ...
  void benchmark_neighbor_atype_throughput(const int num_threads,
                                           const std::string& master_hostname,
                                           std::string warmup_query_file,
                                           std::string query_file) {
  ...
  void benchmark_edge_attrs_latency(std::string res_path, uint64_t WARMUP_N,
                                    uint64_t MEASURE_N,
                                    std::string warmup_query_file,
                                    std::string query_file) {
    time_t t0, t1;
    ...
    std::vector<std::string> result;
    for (uint64_t i = 0; i < WARMUP_N; ++i) {
      get_edge_attrs_f_(result, mod_get(warmup_nhbrAtype_indices, i),
  ...
  void benchmark_edge_attrs_throughput(const int num_threads,
                                       const std::string& master_hostname,
                                       std::string warmup_query_file,
                                       std::string query_file) {
    read_neighbor_atype_queries(warmup_query_file, query_file,
  ...
  // get_neighbor(nodeId, atype)
  void benchmark_neighbor_atype_latency(std::string res_path, uint64_t WARMUP_N,
                                        uint64_t MEASURE_N,
                                        std::string warmup_query_file,
                                        std::string query_file) {
    time_t t0, t1;
    ...
    std::vector<int64_t> result;
    for (uint64_t i = 0; i < WARMUP_N; ++i) {
      get_neighbors_atype_f_(result, mod_get(warmup_nhbrAtype_indices, i),
                             mod_get(warmup_atypes, i));
    }
    LOG_E("Warmup complete.\n");
    ...
    for (uint64_t i = 0; i < MEASURE_N; ++i) {
      t0 = get_timestamp();
      get_neighbors_atype_f_(result, mod_get(nhbrAtype_indices, i),
                             mod_get(atypes, i));
      t1 = get_timestamp();
      res_stream << result.size() << "," << t1 - t0 << "\n";
    ...
    try {
      ...
      while (get_timestamp() - start < WARMUP_MICROSECS) {
        ...
        thread_data->client->get_neighbors(
            result, mod_get(warmup_neighbor_indices, warmup_dis(gen)));
        ++i;
    ...
    try {
      ...
      while (get_timestamp() - start < WARMUP_MICROSECS) {
        ...
        thread_data->client->get_neighbors_atype(
            result, mod_get(warmup_nhbrAtype_indices, query_idx),
      ...
      while (get_timestamp() - start < MEASURE_MICROSECS) {
        ...
        thread_data->client->get_neighbors_atype(
            result, mod_get(nhbrAtype_indices, query_idx),
    ...
    try {
      ...
      while (get_timestamp() - start < WARMUP_MICROSECS) {
        ...
        thread_data->client->get_neighbors_attr(
            result, mod_get(warmup_nhbrNode_indices, query_idx),
      ...
      while (get_timestamp() - start < MEASURE_MICROSECS) {
        query_idx = measure_dis(gen);
        thread_data->client->get_neighbors_attr(
            result, mod_get(nhbrNode_indices, query_idx),
  ...
  std::pair<double, double> benchmark_neighbor_node2_throughput_helper(
      shared_ptr<benchmark_thread_data_t> thread_data) {
    double query_thput = 0;
    double edges_thput = 0;
    COND_LOG_E("About to start querying on this thread...\n");
    ...
    try {
      ...
      while (get_timestamp() - start < WARMUP_MICROSECS) {
        query_idx = warmup_dis(gen);
        thread_data->client->get_neighbors_attr2(
            result, mod_get(warmup_nhbrNode_indices, query_idx),
            mod_get(warmup_nhbrNode_attr_ids, query_idx),
            mod_get(warmup_nhbrNode_attrs, query_idx));
        ++i;
  ...
  std::pair<double, double> benchmark_node_throughput_helper(
    ...
    try {
      ...
      while (get_timestamp() - start < MEASURE_MICROSECS) {
        query_idx = measure_dis(gen);
        thread_data->client->get_nodes(result,
  ...
  std::pair<double, double> benchmark_node_node_throughput_helper(
    ...
    try {
      ...
      while (get_timestamp() - start < MEASURE_MICROSECS) {
        query_idx = measure_dis(gen);
        thread_data->client->get_nodes2(result,
                                        mod_get(node_attributes, query_idx),
                                        mod_get(node_queries, query_idx),
  ...
  std::pair<double, double> benchmark_node_node2_throughput_helper(
      shared_ptr<benchmark_thread_data_t> thread_data) {
    double query_thput = 0;
    double edges_thput = 0;
    COND_LOG_E("About to start querying on this thread...\n");
    ...
    try {
      ...
      while (get_timestamp() - start < WARMUP_MICROSECS) {
        query_idx = warmup_dis(gen);
        thread_data->client->get_nodes22(
            result, mod_get(warmup_node_attributes, query_idx),
            mod_get(warmup_node_queries, query_idx),
            mod_get(warmup_node_attributes2, query_idx),
            mod_get(warmup_node_queries2, query_idx));
      ...
      while (get_timestamp() - start < MEASURE_MICROSECS) {
        query_idx = measure_dis(gen);
        thread_data->client->get_nodes22(result,
                                         mod_get(node_attributes, query_idx),
                                         mod_get(node_queries, query_idx),
                                         mod_get(node_attributes2, query_idx),
                                         mod_get(node_queries2, query_idx));
  ...
  std::pair<double, double> benchmark_mix_throughput_helper(
    ...
    thread_local std::random_device rd;
    thread_local std::mt19937 gen(rd());
    std::uniform_int_distribution<int> warmup_nhbr_dis(0, warmup_nhbr_size - 1);
    std::uniform_int_distribution<int> warmup_nhbr_node_dis(
        0, warmup_nhbr_node_size);
    std::uniform_int_distribution<int> warmup_node_dis(0, warmup_node_size);
    ...
    try {
      ...
      while (get_timestamp() - start < WARMUP_MICROSECS) {
        ...
        switch (rand_query) {
          case 0:
            thread_data->client->get_neighbors(
                result, mod_get(warmup_neighbor_indices, warmup_nhbr_dis(gen)));
            break;
          case 1:
          ...
          case 2:
            ...
            thread_data->client->get_edge_attrs(
                attrs, mod_get(warmup_nhbrAtype_indices, query_idx),
                mod_get(warmup_atypes, query_idx));
            break;
          case 3:
            query_idx = warmup_nhbr_atype_dis(gen);
          ...
          case 4:
            ...
            thread_data->client->get_nodes2(
                result_set, mod_get(warmup_node_attributes, query_idx),
                mod_get(warmup_node_queries, query_idx),
                mod_get(warmup_node_attributes2, query_idx),
                mod_get(warmup_node_queries2, query_idx));
            break;
...
#define RUN_MIX_THPUT_BODY
        ...
        switch (rand_query) {
          case 0:
            ...
            thread_data->client->get_neighbors(
                result, mod_get(neighbor_indices, query_idx));
            break;
          case 1:
          ...
          case 2:
            ...
            thread_data->client->get_edge_attrs(
                attrs, mod_get(nhbrAtype_indices, query_idx),
                mod_get(atypes, query_idx));
            break;
          case 3:
            query_idx = nhbr_atype_dis(gen);
          ...
          case 4:
            ...
            thread_data->client->get_nodes2(
                result_set, mod_get(node_attributes, query_idx),
                mod_get(node_queries, query_idx),
                mod_get(node_attributes2, query_idx),
                mod_get(node_queries2, query_idx));
            break;
  ...
  void benchmark_neighbor_throughput(
      int num_threads, const std::string& master_hostname,
  ...
  std::pair<double, double> benchmark_tao_assoc_range_throughput_helper(
    ...
    while (get_timestamp() - start < WARMUP_MICROSECS) {
      ...
      thread_data->client->assoc_range(
          result, this->warmup_assoc_range_nodes.at(query_idx),
          this->warmup_assoc_range_atypes.at(query_idx),
          this->warmup_assoc_range_offs.at(query_idx),
          this->warmup_assoc_range_lens.at(query_idx));
  ...
  std::pair<double, double> benchmark_tao_obj_get_throughput_helper(
    ...
    std::uniform_int_distribution<int> obj_get_size(0,
                                                    obj_get_nodes.size() - 1);
    ...
    while (get_timestamp() - start < MEASURE_MICROSECS) {
      ...
      thread_data->client->obj_get(attrs, this->obj_get_nodes.at(query_idx));
      ++i;
    }
  ...
  std::pair<double, double> benchmark_tao_assoc_get_throughput_helper(
    ...
    while (get_timestamp() - start < WARMUP_MICROSECS) {
      ...
      thread_data->client->assoc_get(
          result, this->warmup_assoc_get_nodes.at(query_idx),
          this->warmup_assoc_get_atypes.at(query_idx),
          this->warmup_assoc_get_dst_id_sets.at(query_idx),
          this->warmup_assoc_get_lows.at(query_idx),
          this->warmup_assoc_get_highs.at(query_idx));
  ...
  std::pair<double, double> benchmark_tao_assoc_count_throughput_helper(
      shared_ptr<benchmark_thread_data_t> thread_data) {
    double query_thput = 0;
    double edges_thput = 0;
    COND_LOG_E("About to start querying on this thread...\n");
    ...
    while (get_timestamp() - start < MEASURE_MICROSECS) {
      ...
      thread_data->client->assoc_count(this->assoc_count_nodes.at(query_idx),
    ...
    std::ofstream ofs("throughput_tao_assoc_count.txt",
                      std::ofstream::out | std::ofstream::app);
    ofs << query_thput << " " << edges_thput << std::endl;
    ofs.close();
  ...
  std::pair<double, double> benchmark_tao_assoc_time_range_throughput_helper(
      shared_ptr<benchmark_thread_data_t> thread_data) {
    double query_thput = 0;
    double edges_thput = 0;
    COND_LOG_E("About to start querying on this thread...\n");
    ...
    while (get_timestamp() - start < WARMUP_MICROSECS) {
      ...
      thread_data->client->assoc_time_range(
          result, this->warmup_assoc_time_range_nodes.at(query_idx),
          this->warmup_assoc_time_range_atypes.at(query_idx),
          this->warmup_assoc_time_range_lows.at(query_idx),
          this->warmup_assoc_time_range_highs.at(query_idx),
    ...
    while (get_timestamp() - start < MEASURE_MICROSECS) {
      ...
      thread_data->client->assoc_time_range(
          result, this->assoc_time_range_nodes.at(query_idx),
    ...
    std::ofstream ofs("throughput_tao_assoc_time_range.txt",
                      std::ofstream::out | std::ofstream::app);
    ofs << query_thput << " " << edges_thput << std::endl;
    ofs.close();
      ...
      try {
        switch (query) {
          ...
          case 1:
            ...
            COND_LOG_E("obj_get, query idx %d (%d)\n",
                query_idx, warmup_obj_get_nodes.size());
            thread_data->client->obj_get(
                attrs, this->warmup_obj_get_nodes.at(query_idx));
            break;
          case 2:
            query_idx = warmup_assoc_get_size(gen);
            COND_LOG_E("assoc_get, query idx %d (%d %d %d %d %d)\n",
      ...
      try {
        ...
        switch (query) {
          case 0:
            query_idx = assoc_range_size(gen);
            thread_data->client->assoc_range(
                result, this->assoc_range_nodes.at(query_idx),
                this->assoc_range_atypes.at(query_idx),
                this->assoc_range_offs.at(query_idx),
                this->assoc_range_lens.at(query_idx));
            break;
          ...
          case 2:
            query_idx = assoc_get_size(gen);
            thread_data->client->assoc_get(
                result, this->assoc_get_nodes.at(query_idx),
                this->assoc_get_atypes.at(query_idx),
                this->assoc_get_dst_id_sets.at(query_idx),
                this->assoc_get_lows.at(query_idx),
                this->assoc_get_highs.at(query_idx));
        ...
        switch (query) {
          ...
          case 5:
            ...
            COND_LOG_E("assoc_add(%lld,atype %d,%lld,...) ",
                src, atype, dst);
            ret = thread_data->client->assoc_add(src, atype, dst, MAX_TIME,
                                                 ATTR_FOR_NEW_EDGES);
      ...
      try {
        ...
        switch (query) {
          ...
          case 5:
            ...
            atype = dist_atype(gen);
            dst = dist_node(gen);
            thread_data->client->assoc_add(src, atype, dst, MAX_TIME,
                                           ATTR_FOR_NEW_EDGES);
            break;
          case 6:
            obj = thread_data->client->obj_add(ATTRS_FOR_NEW_NODES);
  ...
  void benchmark_tao_mix_throughput(
      const int num_threads, const std::string& master_hostname,
      const std::string& warmup_assoc_range_file,
      const std::string& assoc_range_file,
      const std::string& warmup_assoc_count_file,
      const std::string& assoc_count_file,
      const std::string& warmup_obj_get_file, const std::string& obj_get_file,
      const std::string& warmup_assoc_get_file,
      const std::string& assoc_get_file,
      const std::string& warmup_assoc_time_range_file,
      const std::string& assoc_time_range_file) {
  ...
  void benchmark_tao_assoc_range_throughput(
      const int num_threads, const std::string& master_hostname,
      const std::string& warmup_assoc_range_file,
      const std::string& assoc_range_file) {
    // assoc_range
    ...
    bench_throughput(num_threads, master_hostname, BenchType::TAO_ASSOC_RANGE);
  }

  void benchmark_tao_assoc_get_throughput(
      const int num_threads, const std::string& master_hostname,
      const std::string& warmup_assoc_get_file,
      const std::string& assoc_get_file) {
    // assoc_get
    ...
    bench_throughput(num_threads, master_hostname, BenchType::TAO_ASSOC_GET);
  }

  void benchmark_tao_obj_get_throughput(const int num_threads,
                                        const std::string& master_hostname,
                                        const std::string& warmup_obj_get_file,
                                        const std::string& obj_get_file) {
    // obj_get
    ...
    bench_throughput(num_threads, master_hostname, BenchType::TAO_OBJ_GET);
  }

  void benchmark_tao_assoc_count_throughput(
      const int num_threads, const std::string& master_hostname,
      const std::string& warmup_assoc_count_file,
      const std::string& assoc_count_file) {
    // assoc_count
    ...
    bench_throughput(num_threads, master_hostname, BenchType::TAO_ASSOC_COUNT);
  }

  void benchmark_tao_assoc_time_range_throughput(
      const int num_threads, const std::string& master_hostname,
      const std::string& warmup_assoc_time_range_file,
      const std::string& assoc_time_range_file) {
    // assoc_time_range
    ...
    bench_throughput(num_threads, master_hostname,
                     BenchType::TAO_ASSOC_TIME_RANGE);
  }

  void benchmark_node_throughput(const int num_threads,
                                 const std::string& master_hostname,
                                 std::string warmup_query_file,
                                 std::string query_file) {
  ...
  void benchmark_node_latency(std::string res_path, uint64_t WARMUP_N,
                              uint64_t MEASURE_N, std::string warmup_query_file,
                              std::string query_file) {
    time_t t0, t1;
    read_node_queries(warmup_query_file, query_file);
    std::ofstream res_stream(res_path);
    ...
    std::set<int64_t> result;
    for (uint64_t i = 0; i < WARMUP_N; ++i) {
      get_nodes_f_(result, mod_get(warmup_node_attributes, i),
  ...
  void benchmark_node_node_throughput(const int num_threads,
                                      const std::string& master_hostname,
  ...
  void benchmark_node_node2_throughput(const int num_threads,
                                       const std::string& master_hostname,
                                       const std::string& warmup_query_file,
                                       const std::string& query_file) {
    read_node_queries(warmup_query_file, query_file);
    bench_throughput(num_threads, master_hostname, BenchType::NODE22);
  }

  void benchmark_node_node_latency(std::string res_path, uint64_t WARMUP_N,
                                   uint64_t MEASURE_N,
                                   const std::string& warmup_query_file,
                                   const std::string& query_file) {
    read_node_queries(warmup_query_file, query_file);
    ...
    std::set<int64_t> result;
    for (uint64_t i = 0; i < WARMUP_N; ++i) {
      get_nodes2_f_(result, mod_get(warmup_node_attributes, i),
                    mod_get(warmup_node_queries, i),
  ...
  void benchmark_neighbor_node_throughput(const int num_threads,
                                          const std::string& master_hostname,
                                          std::string warmup_query_file,
                                          std::string query_file) {
  ...
  void benchmark_neighbor_node2_throughput(const int num_threads,
                                           const std::string& master_hostname,
                                           std::string warmup_query_file,
                                           std::string query_file) {
    read_neighbor_node_queries(warmup_query_file, query_file);
    bench_throughput(num_threads, master_hostname, BenchType::NHBR_NODE2);
  }

  void benchmark_neighbor_node_latency(std::string res_path, uint64_t WARMUP_N,
                                       uint64_t MEASURE_N,
                                       std::string warmup_query_file,
                                       std::string query_file) {
    read_neighbor_node_queries(warmup_query_file, query_file);
    ...
    std::vector<int64_t> result;
    for (uint64_t i = 0; i < WARMUP_N; ++i) {
      get_neighbors_attr_f_(result, mod_get(warmup_nhbrNode_indices, i),
    ...
    for (uint64_t i = 0; i < MEASURE_N; ++i) {
      t0 = get_timestamp();
      get_neighbors_attr_f_(result, mod_get(nhbrNode_indices, i),
  ...
  void benchmark_tao_assoc_add_latency(
      const std::string& assoc_add_res_file, int max_num_new_edges =
          MAX_NUM_NEW_EDGES) {
    ...
    try {
      ...
      for (int i = 0; i < warmup_n; ++i) {
        ...
        atype = dist_atype(gen);
        dst = dist_node(gen);
        aggregator_->assoc_add(src, atype, dst, MAX_TIME, ATTR_FOR_NEW_EDGES);
      }
      LOG_E("Warmup complete.\n");
      ...
      for (int i = 0; i < measure_n; ++i) {
        ...
        t0 = get_timestamp();
        ret = aggregator_->assoc_add(src, atype, dst, MAX_TIME,
                                     ATTR_FOR_NEW_EDGES);
        t1 = get_timestamp();
  ...
  void benchmark_tao_obj_add_latency(
      const std::string& obj_add_res_file, int max_num_new_nodes =
          MAX_NUM_NEW_NODES) {
  ...
  void benchmark_tao_mix_latency(
      ...
      const std::string& assoc_time_range_res_file, int warmup_n, int measure_n,
      const std::string& warmup_assoc_range_file,
      const std::string& assoc_range_file,
      const std::string& warmup_assoc_count_file,
      const std::string& assoc_count_file,
    ...
    try {
      ...
      for (int i = 0; i < warmup_n; ++i) {
        ...
        switch (rand_query) {
          case 0:
            assoc_range_f_(result, mod_get(warmup_assoc_range_nodes, i),
                           mod_get(warmup_assoc_range_atypes, i),
                           mod_get(warmup_assoc_range_offs, i),
                           mod_get(warmup_assoc_range_lens, i));
            break;
          case 1:
            assoc_count_f_(mod_get(warmup_assoc_count_nodes, i),
          ...
          case 2:
            ...
            break;
          case 3:
            assoc_get_f_(result, mod_get(warmup_assoc_get_nodes, i),
      ...
      for (int i = 0; i < measure_n; ++i) {
        ...
        switch (rand_query) {
          case 0:
            t0 = get_timestamp();
            assoc_range_f_(result, mod_get(assoc_range_nodes, i),
          ...
          case 1:
            t0 = get_timestamp();
            cnt = assoc_count_f_(mod_get(assoc_count_nodes, i),
          ...
          case 3:
            t0 = get_timestamp();
            assoc_get_f_(result, mod_get(assoc_get_nodes, i),
          ...
          case 4:
            t0 = get_timestamp();
            assoc_time_range_f_(result, mod_get(assoc_time_range_nodes, i),
  ...
  void benchmark_tao_mix_with_updates_latency(
      ...
      const std::string& obj_add_res_file, int warmup_n, int measure_n,
      const std::string& warmup_assoc_range_file,
      const std::string& assoc_range_file,
      const std::string& warmup_assoc_count_file,
      const std::string& assoc_count_file,
    ...
    try {
      LOG_E("Warming up for %d queries...\n", warmup_n);
      for (int i = 0; i < warmup_n; ++i) {
        int rand_query = choose_query_with_updates(query_dis(rng),
                                                   query_dis(rng));
        switch (rand_query) {
          case 0:
            assoc_range_f_(result, mod_get(warmup_assoc_range_nodes, i),
  ...
  void benchmark_mix_throughput(const int num_threads,
                                const std::string& master_hostname,
                                const std::string& warmup_neighbor_query_file,
                                const std::string& neighbor_query_file,
  ...
  void benchmark_mix_latency(const std::string& nhbr_res_file,
    ...
    try {
      ...
      for (int i = 0; i < WARMUP_N; ++i) {
        ...
        switch (rand_query) {
          ...
          case 1:
            ...
            break;
          case 2:
            get_nodes_f_(result_set, mod_get(warmup_node_attributes, i),
                         mod_get(warmup_node_queries, i));
            break;
          case 3:
            get_neighbors_atype_f_(result, mod_get(warmup_nhbrAtype_indices, i),
            ...
            break;
          case 4:
            get_nodes2_f_(result_set, mod_get(warmup_node_attributes, i),
                          mod_get(warmup_node_queries, i),
                          mod_get(warmup_node_attributes2, i),
                          mod_get(warmup_node_queries2, i));
            break;
      ...
      for (int i = 0; i < MEASURE_N; ++i) {
        ...
        switch (rand_query) {
          ...
          case 2: {
            scoped_timer t(&latency);
            get_nodes_f_(result_set, mod_get(node_attributes, i),
                         mod_get(node_queries, i));
          }
          ...
          case 4: {
            scoped_timer t(&latency);
            get_nodes2_f_(result_set, mod_get(node_attributes, i),
                          mod_get(node_queries, i),
  ...
  // TAO Queries
  void benchmark_assoc_range_latency(std::string res_path, uint64_t warmup_n,
                                     uint64_t measure_n,
                                     const std::string& warmup_query_file,
                                     const std::string& query_file) {
  ...
  void benchmark_assoc_count_latency(std::string res_path, uint64_t warmup_n,
                                     uint64_t measure_n,
                                     const std::string& warmup_query_file,
                                     const std::string& query_file) {
    read_neighbor_atype_queries(warmup_query_file, query_file,
  ...
  void benchmark_assoc_get_latency(std::string res_path, int64_t warmup_n,
                                   int64_t measure_n,
                                   const std::string& warmup_query_file,
                                   const std::string& query_file) {
    ...
    for (int64_t i = 0; i < measure_n; ++i) {
      ...
      assoc_get_f_(result, mod_get(assoc_get_nodes, i),
                   mod_get(assoc_get_atypes, i),
                   mod_get(assoc_get_dst_id_sets, i),
                   mod_get(assoc_get_lows, i), mod_get(assoc_get_highs, i));
      t1 = get_timestamp();
      res_stream << result.size() << "," << t1 - t0 << "\n";
  ...
  void benchmark_obj_get_latency(std::string res_path, uint64_t warmup_n,
                                 uint64_t measure_n,
                                 const std::string& warmup_query_file,
                                 const std::string& query_file) {
    read_neighbor_queries(warmup_query_file, query_file, warmup_obj_get_nodes,
                          obj_get_nodes);
    time_t t0, t1;
    std::ofstream res_stream(res_path);
  ...
  void benchmark_assoc_time_range_latency(std::string res_path,
                                          int64_t warmup_n, int64_t measure_n,
                                          const std::string& warmup_query_file,
                                          const std::string& query_file) {
    ...
    std::vector<ThriftAssoc> result;
    for (int64_t i = 0; i < warmup_n; ++i) {
      assoc_time_range_f_(result, mod_get(warmup_assoc_time_range_nodes, i),
                          mod_get(warmup_assoc_time_range_atypes, i),
                          mod_get(warmup_assoc_time_range_lows, i),
                          mod_get(warmup_assoc_time_range_highs, i),
                          mod_get(warmup_assoc_time_range_limits, i));
 ...
 protected:
  ...
  std::function<void(std::vector<int64_t>&, int64_t, int64_t)> get_neighbors_atype_f_;

  std::function<void(std::vector<int64_t>&, int64_t, int, const std::string&)> get_neighbors_attr_f_;

  std::function<void(std::set<int64_t>&, int, const std::string&)> get_nodes_f_;

  std::function<
      void(std::set<int64_t>&, int, const std::string&, int, const std::string&)> get_nodes2_f_;

  std::function<void(std::vector<std::string>&, int64_t, int64_t)> get_edge_attrs_f_;
  ...
  std::function<
      void(std::vector<ThriftAssoc>&, int64_t, int64_t, int32_t, int32_t)> assoc_range_f_;

  std::function<
      void(std::vector<ThriftAssoc>&, int64_t, int64_t,
           const std::set<int64_t>&, int64_t, int64_t)> assoc_get_f_;

  std::function<int64_t(int64_t, int64_t)> assoc_count_f_;

  std::function<
      void(std::vector<ThriftAssoc>&, int64_t, int64_t, int64_t, int64_t,
           int32_t)> assoc_time_range_f_;
  ...
  void read_assoc_range_queries(const std::string& warmup_file,
    ...
    read(file, assoc_range_nodes, assoc_range_atypes, assoc_range_offs,
         assoc_range_lens);
  }

  void read_assoc_get_queries(const std::string& warmup_file,
                              const std::string& file) {
    auto read = [](
        const std::string& file,
        std::vector<int64_t>& nodes, std::vector<int64_t>& atypes,
    ...
    read(file, assoc_get_nodes, assoc_get_atypes, assoc_get_lows,
         assoc_get_highs, assoc_get_dst_id_sets);
  }

  void read_assoc_time_range_queries(const std::string& warmup_file,
                                     const std::string& file) {
    auto read = [](
        const std::string& file,
        std::vector<int64_t>& nodes, std::vector<int64_t>& atypes,
    ...
    read(warmup_file, warmup_assoc_time_range_nodes,
         warmup_assoc_time_range_atypes, warmup_assoc_time_range_lows,
         warmup_assoc_time_range_highs, warmup_assoc_time_range_limits);
  ...
  void read_neighbor_queries(const std::string& warmup_neighbor_file,
  ...
  void read_neighbor_atype_queries(
      ...
      std::vector<int64_t>& warmup_atypes, std::vector<int64_t>& atypes) {
  ...
  void read_node_queries(const std::string& warmup_query_file,
                         const std::string& query_file) {
    ...
    while (getline(warmup_input, line)) {
      std::vector<std::string> toks = split(line,
                                            GraphFormatter::QUERY_FILED_DELIM);
  ...
  void read_neighbor_node_queries(const std::string& warmup_query_file,
                                  const std::string& query_file) {
    std::ifstream warmup_input(warmup_query_file);
    std::ifstream query_input(query_file);
