...
void SuccinctGraph::load_deleted_edges(std::string deleted_edges_file) {
  ...
  uint8_t* data = (uint8_t*) SuccinctUtils::MemoryMapMutable(
      deleted_edges_file);
...
std::string SuccinctGraph::mk_edge_table_search_key(int64_t src,
  ...
  std::string atype_str = std::to_string(atype);
  if (atype == INT64_MIN)
    atype_str = "-" + std::to_string(0);
  return key + std::to_string(src) + ATYPE_DELIM + atype_str
...
std::vector<SuccinctGraph::Assoc> SuccinctGraph::assoc_get(
  ...
  COND_LOG_E(
      "assoc_get(src = %" PRId64 ", atype = %" PRId64 "," " dstIdSet = ..., tLow = %" PRId64 ", tHigh = %" PRId64 ")\n",
...
bool SuccinctGraph::deleteNode(int64_t id) {
  COND_LOG_E(
      "[SUCCINCTSTORE] Received request for deleteNode(id=%lld), forwarding to node table.\n",
      id);
...
bool SuccinctGraph::getLink(Link& link, int64_t id1, int64_t link_type,
  ...
  for (int64_t curr_off : eoffs) {
    ...
    if (idx == cnt || deleted_edges->IsDeleted(id1, link_type, idx)) {
      link.src_id = -1;
      link.atype = -1;
      link.dst_id = -1;
      COND_LOG_E("Edge not present or deleted.\n");
      continue;
    }

    COND_LOG_E("Found edge at idx = %lld; invalid_edge_bit=%u\n", idx,
        deleted_edges->IsDeleted(id1, link_type, idx));
...
bool SuccinctGraph::deleteLink(int64_t id1, int64_t link_type, int64_t id2) {
  ...
  for (int64_t curr_off : eoffs) {
    ...
    if (!deleted_edges->IsDeleted(id1, link_type, idx)) {
      deleted_edges->Delete(id1, link_type, idx);
      return true;
    }
  }
...
void SuccinctGraph::getLinkList(std::vector<Link>& assocs, int64_t id1,
  ...
  for (int64_t curr_off : eoffs) {
    ...
    for (size_t i = 0; i < cnt; ++i) {
      ...
      if (deleted_edges->IsDeleted(id1, link_type, i)) {
        assocs.pop_back();
      }
    }
  }
...
void SuccinctGraph::getLinkList(std::vector<Link>& assocs, int64_t id1,
                                ...
                                int64_t limit) {
  COND_LOG_E(
      "getLinkList(id1=%lld, link_type=%lld, min_timestamp=%lld, max_timestamp=%lld, offset=%lld, limit=%lld)\n",
...
void SuccinctGraph::init_rpq_ctx(SuccinctGraph::edge_label label,
  ...
  EDGE_TABLE->Search(offs, search_key);
  COND_LOG_E("[SuccinctGraph] num offs = %zu\n", offs.size());
  for (int64_t off : offs) {
    off += search_key.size();

    std::string str;
    uint64_t idx_hint = -1ULL;
    int32_t dst_id_width, timestamp_width, cnt;
    ...
    // Get dst node ids
    EDGE_TABLE->Extract(str, off, cnt * dst_id_width);
    std::vector<int64_t> decoded_dst_ids =
        SuccinctGraphSerde::decode_multi_node_ids(str, dst_id_width);
