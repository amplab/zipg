...
void GraphFormatter::create_random_node_table(const std::string& out_file,
                                              int num_nodes, int num_attr,
                                              int freq, int len) {
  ...
  for (int i = 0; i < num_nodes; i++) {
    for (int attr = 0; attr < num_attr; attr++) {
      s_out << SuccinctGraph::DELIMITERS[attr] << attributes[attr][i];
    }
    s_out << "\n";
  }
  s_out.close();
...
void GraphFormatter::create_node_table_zipf(const std::string& out_file,
                                            const std::string& attr_file,
                                            int num_nodes, int num_attr,
                                            int len, int corpus_size,
...
void GraphFormatter::create_node_table(const std::string& out_file,
                                       const std::string& attr_file,
                                       int num_nodes, int num_attr, int freq,
                                       int len) {
  assert(num_attr <= SuccinctGraph::MAX_NUM_NODE_ATTRS);
  ...
  assert(
      attributes.size() == static_cast<size_t>(num_attr)
...
std::vector<std::vector<int64_t>> GraphFormatter::read_edge_list(
    const std::string& file, char edge_inner_delim, char edge_end_delim) {
...
void GraphFormatter::read_assoc_list(
    ...
    std::unordered_set<std::pair<int64_t, int64_t>,
        boost::hash<std::pair<int, int> > >& assoc_lists) {
  std::ifstream in_stream(file);
  std::string line, token;
  while (std::getline(in_stream, line)) {
    std::stringstream ss(line);
...
void GraphFormatter::make_rand_assoc(
    SuccinctGraph::Assoc& assoc, int64_t src_id, int64_t dst_id,
    const std::string& attr_file, std::ifstream& attr_in_stream,
    int bytes_per_attr, std::uniform_int_distribution<int64_t> atype_dis,
    std::mt19937& atype_rng, std::uniform_int_distribution<int> time_dis,
    std::mt19937& time_rng, bool augmented_assoc,
...
void GraphFormatter::make_rand_assoc(
    SuccinctGraph::Assoc& assoc, int64_t src_id, int64_t atype,
    const std::string& attr_file, std::ifstream& attr_in_stream,
    int bytes_per_attr, std::uniform_int_distribution<int64_t> node_dis,
    std::uniform_int_distribution<int64_t> time_dis, std::mt19937& rng) {
...
void GraphFormatter::create_edge_table(const std::string& file,
                                       const std::string& attr_file,
                                       const std::string& out_file,
                                       int bytes_per_attr,
                                       char edge_inner_delim,
                                       char edge_end_delim, int num_atype,
                                       int min_out_degree,
  ...
  std::uniform_int_distribution<int> time_dis(0,
  ...
  while (std::getline(in_stream, line)) {
    ...
    GraphFormatter::make_rand_assoc(assoc, src_id, dst_id, attr_file,
                                    attr_in_stream, bytes_per_attr, atype_dis,
                                    rng1, time_dis, rng2);
    ...
    for (int64_t node_id = 0; node_id <= max_node; ++node_id) {
      ...
      while (edge_list_set[node_id].size() < min_out_degree) {
        ...
        if (edge_list_set[node_id].count(dst_id) == 0) {
          ...
          SuccinctGraph::Assoc assoc;
          GraphFormatter::make_rand_assoc(assoc, node_id, dst_id, attr_file,
                                          attr_in_stream, bytes_per_attr,
                                          atype_dis, rng1, time_dis, rng2, true,
                                          assign_ts_attr_to_dummy_edges);
          assoc_map[std::make_pair(node_id, assoc.atype)].push_back(assoc);
        }
      }
      total_edges += edge_list_set[node_id].size();
    }
    LOG_E(
        "Augmentation done, #nodes %" PRId64 ", #edges %" PRId64 ", " "avg deg %.1f\n",
  ...
  for (auto it = assoc_map.begin(); it != assoc_map.end(); ++it) {
    ...
    for (auto it2 = assoc_list.begin(); it2 != assoc_list.end(); ++it2) {
      auto assoc = *it2;
      out_stream << src_id_and_atype.first << " " << assoc.dst_id << " "
                 << src_id_and_atype.second << " " << assoc.time << " "
                 << assoc.attr << "\n";
    }
  }
  out_stream.close();
...
std::string GraphFormatter::format_node_attrs_str(
  ...
  for (auto& attrs : node_attrs) {
    ...
    for (size_t i = 0; i < attrs.size(); ++i) {
      assert(
          attrs[i].find(static_cast<char>(SuccinctGraph::DELIMITERS[i]))
...
void GraphFormatter::format_neo4j_node_from_node_file(
    const std::string& delimed_node_file, const std::string& neo4j_node_out,
    char neo4j_delim) {
...
void GraphFormatter::format_neo4j_edge_from_edge_file(
    const std::string& delimed_edge_file, const std::string& neo4j_edge_out,
    char neo4j_delim) {
...
std::string GraphFormatter::attach_attr_lengths(const std::string& delimed) {
  ...
  std::string out_line;
  out_line += std::to_string(distance) + SuccinctGraph::NODE_TABLE_HEADER_DELIM;
  for (auto len : attr_lengths) {
    out_line += std::to_string(len) + SuccinctGraph::NODE_TABLE_HEADER_DELIM;
  }
  out_line += delimed;
...
void GraphFormatter::build_assoc_map(
    std::map<SuccinctGraph::AssocListKey, std::vector<SuccinctGraph::Assoc>>& assoc_map,
    const std::string& in) {
  assoc_map.clear();
...
void GraphFormatter::build_edge_updates(
    ...
    const std::string& assoc_in, const int num_shards_to_mod) {
  edge_updates.clear();
...
void GraphFormatter::populate_random_store(const std::string& store_out,
                                           size_t num_edges_to_add,
                                           size_t num_nodes, size_t num_atypes,
                                           AssocSet& set,
                                           const std::string& attr_file,
                                           int bytes_per_attr, int64_t min_time,
                                           int64_t max_time) {
  std::random_device rd;
  std::mt19937 rng(rd());
  ...
  for (size_t i = 0; i < num_edges_to_add; ++i) {
    ...
    GraphFormatter::make_rand_assoc(assoc, src, atype, attr_file, attr_in,
                                    bytes_per_attr, uni_node, uni_time, rng);

    out << assoc.src_id << ' ' << assoc.dst_id << ' ' << assoc.atype << ' '
        << assoc.time << ' ' << assoc.attr << std::endl;
  }
  out.flush();
  LOG_E("Store input written out to: '%s'\n", store_out.c_str());
}

void GraphFormatter::make_rand_suffix_store(const std::string& store_out,
                                            size_t num_edges_to_add,
                                            size_t num_nodes, size_t num_atypes,
  ...
  if (!file_or_dir_exists(store_out)) {
    std::unordered_set<std::pair<int64_t, int64_t>,
        boost::hash<std::pair<int, int> > > set;
    read_assoc_set(set, assoc_set_file);
    LOG_E("read in assoc set, size: %lld\n", set.size());

    populate_random_store(store_out + "_edgelist", num_edges_to_add, num_nodes,
                          num_atypes, set, attr_file, bytes_per_attr, min_time,
                          max_time);
...
void GraphFormatter::make_rand_log_store(const std::string& store_out,
                                         size_t num_edges_to_add,
                                         size_t num_nodes, size_t num_atypes,
                                         const std::string& assoc_set_file,
                                         const std::string& attr_file,
  ...
  if (!file_or_dir_exists(store_out)) {
    ...
    populate_random_store(store_out, num_edges_to_add, num_nodes, num_atypes,
                          set, attr_file, bytes_per_attr, min_time, max_time);
  }
...
void GraphFormatter::read_assoc_set(
    std::unordered_set<std::pair<int64_t, int64_t>,
        boost::hash<std::pair<int, int> > >& set,
    const std::string& assoc_set_file) {
