...
if (THRIFT_COMPILER)
    ...
    function(thrift_gen_java thrift_file dst)
        if(EXISTS ${thrift_file})
            get_filename_component(_target_dir ${thrift_file} NAME_WE)
            message(STATUS "Generating thrift java files for ${_target_dir}")
            if(NOT EXISTS ${CMAKE_BINARY_DIR}/${_target_dir})
                file(MAKE_DIRECTORY ${CMAKE_BINARY_DIR}/${_target_dir})
            endif()
            execute_process(COMMAND ${THRIFT_COMPILER} -o "${CMAKE_BINARY_DIR}/${_target_dir}" --gen java ${thrift_file})
        else()
            message(SEND_ERROR "thrift_gen_java: File ${thrift_file} does not exist")
        endif()
        set(src "${CMAKE_BINARY_DIR}/${_target_dir}/gen-java")
        file(REMOVE_RECURSE ${dst})
        message(STATUS "Copying ${src} to ${dst}...")
        file(RENAME ${src} ${dst})
    endfunction()
    function(thrift_gen_py thrift_file dst)
        if(EXISTS ${thrift_file})
            get_filename_component(_target_dir ${thrift_file} NAME_WE)
            message(STATUS "Generating thrift python files for ${_target_dir}")
            if(NOT EXISTS ${CMAKE_BINARY_DIR}/${_target_dir})
                file(MAKE_DIRECTORY ${CMAKE_BINARY_DIR}/${_target_dir})
            endif()
            execute_process(COMMAND ${THRIFT_COMPILER} -o "${CMAKE_BINARY_DIR}/${_target_dir}" --gen py ${thrift_file})
        else()
            message(SEND_ERROR "thrift_gen_py: File ${thrift_file} does not exist")
        endif()
        set(src "${CMAKE_BINARY_DIR}/${_target_dir}/gen-py")
        file(REMOVE_RECURSE ${dst})
        message(STATUS "Copying ${src} to ${dst}...")
        file(RENAME ${src} ${dst})
    endfunction()
