...
bool SuccinctShard::Delete(int64_t key) {
  ...
  COND_LOG_E("invalid_offsets_->bitmap = %p", invalid_offsets_->bitmap);
...
void SuccinctShard::SetKeys(const std::vector<int64_t> keys) {
  assert(keys_.size() == keys.size() && "# new keys != # old keys!");
  keys_ = std::move(keys);
}
